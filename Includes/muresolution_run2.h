 ...
 //private:
  ...
  double kSmear(double pt, double eta, int nlayers=0, TYPE type=Extra){
	...
	if(RD>RM) x = sqrt(RD*RD-RM*RM)*cb[H][F].invcdf(u);
      ...
      else		    x = Sigma(pt, H, F)*cb[H][F].invcdf(u);
      //else		    x = kRes[H]*Sigma(pt, H, F)*cb[H][F].invcdf(u); //for next iteration
    }
  ...
  //static constexpr double BETA[NETA+1];
  ...
  //static constexpr double kDat[NETA];
  const double kDat[12] = {1.05329,1.08899,1.08421,1.03354,1.08724,1.11278,1.06977,1.02507,1.02694,1.1133,1.10883,1.12218};
  //static constexpr double kRes[NETA];
  const double kRes[12] = {1.00598,1.03484,0.992156,0.947819,0.937742,0.939444,0.941247,0.933301,0.945896,1.01797,0.972806,0.998117};
